...
export function dump(metadata, file, options = {}) {
    assertFlacFile(file);
    const blocks = [];
    blocks.push({
    ...
    return concat(Uint8Array.of(102, 76, 97, 67), // "fLaC"
    ...blocks.map(({ type, bytes }, index, blocks) => dumpBlock(index === blocks.length - 1, type, bytes)), file.subarray(skipMetadata(file)));
}
function dumpBlock(isLastBlock, type, data) {
...
function dumpStreamInfo(streamInfo) {
    ...
    return concat(dumpNumber(streamInfo.minBlockSize, 2), dumpNumber(streamInfo.maxBlockSize, 2), dumpNumber(streamInfo.minFrameSize, 3), dumpNumber(streamInfo.maxFrameSize, 3), Uint8Array.of((streamInfo.sampleRate >> 12) & 0xFF, (streamInfo.sampleRate >> 4) & 0xFF, ((streamInfo.sampleRate & 0xF) << 4) // sample rate low 4 bity (high nibble bajtu 12)
        | (((streamInfo.numberOfChannels - 1) & 0x7) << 1) // channels (3 bity)
        | (((streamInfo.bitsPerSample - 1) >> 4) & 0x1), (((streamInfo.bitsPerSample - 1) & 0xF) << 4) // upper 4 bits bitsPerSample in high nibble
        | Number((totalSamples >> 32n) & 0xfn), // highest 4 bits totalSamples in low nibble
    Number((totalSamples >> 24n) & 0xffn), Number((totalSamples >> 16n) & 0xffn), Number((totalSamples >> 8n) & 0xffn), Number(totalSamples & 0xffn)), md5);
}
const textEncoder = new TextEncoder();
function dumpVorbisComment(vorbisComment) {
    ...
    return concat(dumpVorbisCommentLength(vendor.length), vendor, dumpVorbisCommentLength(vorbisComment.comments.length), ...comments);
}
function dumpVorbisCommentLength(length) {
    return Uint8Array.of(length, length >> 8, length >> 16, length >> 24);
}
function dumpPicture(picture) {
    ...
    return concat(type, dumpNumber(mime.length, 4), mime, dumpNumber(description.length, 4), description, dumpNumber(picture.width, 4), dumpNumber(picture.height, 4), dumpNumber(picture.colorDepth, 4), dumpNumber(picture.usedColors, 4), dumpNumber(picture.picture.length, 4), picture.picture);
}
function dumpNumber(n, length) {
